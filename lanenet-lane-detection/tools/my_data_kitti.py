    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            for j in range(len(image_path_epoch)):          
                H, W, C = image_list_epoch[j].shape
                diff_H = H - size_H
                diff_W = W - size_W
                img_crop = image_list_epoch[j][diff_H::,diff_W//2: diff_W//2 + size_W, :] 
                img_list.append(img_crop)
                
                img_left_list.append(image_list_epoch[j][diff_H::,0:size_W, :])
                
                img_right_list.append(image_list_epoch[j][diff_H::, diff_W:: , :])
                
                
                print (image_path_epoch[j])
                print (image_path_epoch[j].replace('/test_data_image/','/test_data_depth/'))
                sparse = cv2.imread(image_path_epoch[j].replace('/test_data_image/','/test_data_depth/') ,-1)
                sparse_list.append(sparse)
                sparse = np.array(sparse, dtype = np.float32)/255
                
                spa = sparse[diff_H::,diff_W//2: diff_W//2 + size_W]
                ...
                spa_left = sparse[diff_H::,0:size_W]
                spa_left = spa_left[..., np.newaxis]
                spa_left_list.append(spa_left)


                spa_right = sparse[diff_H::, diff_W::]
                spa_right = spa_right[..., np.newaxis]                
                spa_right_list.append(spa_right)                
            ...
            pred_result = sess.run([pred],
                                                        feed_dict={input_tensor1: img_list,
                                                        input_tensor2: spa_list
                                                        })            
            pred_result_left = sess.run([pred],
                                                        feed_dict={input_tensor1: img_left_list,
                                                        input_tensor2: spa_left_list
                                                        })  

            pred_result_right = sess.run([pred],
                                                        feed_dict={input_tensor1: img_right_list,
                                                        input_tensor2: spa_right_list
                                                        })                                                         
            t_cost = time.time() - t_start
            ...
            for j in range(len(image_path_epoch)):
                ...
                pred_result_each_left = pred_result_left[j][0]
                pred_result_each_right = pred_result_right[j][0]
                
                sparse_list[j]
                H, W = sparse_list[j].shape
                diff_H = H - size_H
                diff_W = W - size_W 
                #mask_spa = sparse_list[j][diff_H::,diff_W//2: diff_W//2 + size_W]
                ...
                pred_result_each_left = np.array(pred_result_each_left*255, dtype = np.uint16)
                pred_result_each_right = np.array(pred_result_each_right*255, dtype = np.uint16)
                
                
                
                sparse_list[j][diff_H::,0: size_W] = pred_result_each_left[:,:,0]
                
                sparse_list[j][diff_H::,diff_W::] = pred_result_each_right[:,:,0]
                
                sparse_list[j][diff_H::,diff_W//2: diff_W//2 + size_W] = pred_result_each[:,:,0] 
                

                cv2.imwrite(image_save_path, sparse_list[j])
