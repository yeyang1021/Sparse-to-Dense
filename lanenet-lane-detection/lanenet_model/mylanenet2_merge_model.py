...
from encoder_decoder_model import refcn_decoder
...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    def __init__(self, phase, net_flag='resnet'):
        ...
        if self._net_flag == 'vgg':
            ...
            self._encoder = dense_encoder.DenseEncoder(l=20, growthrate=8,
                                                       ...
                                                       phase=self._phase,
                                                       n=5)
        self._decoder =refcn_decoder.FCNDecoder()
        return
    ...
    def compute_loss(self, input_tensor1, input_tensor2, gt_label, mask_label, name):
        ...
        with tf.variable_scope(name):
            ...
            refine_logits = inference_ret['refine']
            decode_logits = decode_logits * mask_label
            
            
            gt_refine = (decode_logits - gt_label)*255
            ...
            dense_loss2 = tf.losses.absolute_difference(
                gt_refine,
                refine_logits,
                weights=1.0,
                scope=None,
            ...
            ret = {
                'total_loss': dense_loss + dense_loss1 + dense_loss2,
            }
